...
function addTab(subtitle, url, icon) {
	if (!$('#tabs').tabs('exists', subtitle)) {
		$('#tabs').tabs('add', {
			...
			icon : icon
		/*
		 * ,tools : [ { iconCls : 'icon-help', handler : function() { var last =
		 * url.lastIndexOf("/"); var helpUrl = url.substring(0, last) +
		 * "/help.html"; window.open(helpUrl); } } ]
		 */
		});
	} else {
...
function closeTab(action) {
	...
	$('#mm-tabupdate').click(function() {
		...
		var a = currTab.panel("options").tools;
		$('#tabs').tabs('update', {
			tab : currTab,
			tools : [],
			options : {
